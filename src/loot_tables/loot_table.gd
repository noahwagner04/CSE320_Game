...
func set_non_exported_vals():
	# ensure max >= min
	if max_weapon_drops < min_weapon_drops:
		max_weapon_drops = min_weapon_drops
	if max_health_potion_drops < min_health_potion_drops:
		max_health_potion_drops = min_health_potion_drops
	weapon_t1_dict = {
		"tier": 1, 
